...
function modifyArray(array) { for(const index in array)
...
function findMinMax([firstElement, ...restElements]) {
  ...
  {if(restElements[i]<min){ min=restElements[i];} if(restElements[i]>max){max=restElements[i];}}
...
function printStrings(array) { const str1=array.toString(); const str2=str1.trim();
...
function swapMinMax(array) {
  ...
  {if(array[i]>max){max=array[i]; maxIndex=i;}
   else if(array[i]<min){min=array[i]; minIndex=i;}}
...
function replaceNumbers(arr) {
  ...
  // Проходимо через кожен елемент вхідного масиву
  for(let i=0; i<arr.length; i++){
    if (arr[i]>10){result = [...result, "Greater than 10"];}
